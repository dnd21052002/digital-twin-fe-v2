...
function kpiStatusColor(kpi: KpiData): { bg: string; text: string; border: string } {
  if (kpi.status === 'critical') return { bg: 'bg-critical/5', text: 'text-critical', border: 'border-critical/30' };
  if (kpi.status === 'warning') return { bg: 'bg-warning/5', text: 'text-warning', border: 'border-warning/30' };
  if (kpi.status === 'good') return { bg: 'bg-success/5', text: 'text-success', border: 'border-success/30' };
  const v = kpi.value;
  const t = kpi.target;
  if (t > 0 && v > t) return { bg: 'bg-critical/5', text: 'text-critical', border: 'border-critical/30' };
  if (t > 0 && v > t * 0.8) return { bg: 'bg-warning/5', text: 'text-warning', border: 'border-warning/30' };
  return { bg: 'bg-success/5', text: 'text-success', border: 'border-success/30' };
}
...
export function KpiCardSection() {
  ...
  const content = useMemo(() => {
    ...
    return <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{items.map((kpi) => {
      const colors = kpiStatusColor(kpi);
      const vsTarget = kpi.target > 0 ? ((kpi.value - kpi.target) / kpi.target * 100).toFixed(1) : null;
      return (
        <article key={kpi.key} className={`rounded-lg border ${colors.border} ${colors.bg} p-4`}>
          <div className="flex items-start justify-between gap-2">
            <p className="text-eyebrow text-ink-subtle">{kpi.name}</p>
            <span className={`text-caption font-medium ${colors.text}`}>{kpi.status ?? '—'}</span>
          </div>
          <p className={`mt-2 font-mono text-headline tabular-nums ${colors.text}`}>
            {kpi.value}<span className="ml-1 text-body-sm text-ink-muted">{kpi.unit}</span>
          </p>
          {vsTarget !== null && (
