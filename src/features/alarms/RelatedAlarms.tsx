...
export function RelatedAlarms({ assetId, alarms }: RelatedAlarmsProps) {
  ...
  return (
    <section role="region" aria-label="Related alarms" className="rounded-xl border border-border-subtle bg-bg-panel p-4">
      <h3 className="text-sm font-semibold text-text-primary">Related alarms</h3>
      {visible.length === 0 ? <div className="mt-3"><EmptyState title="No related alarms" message="No active alarms are linked to this asset." /></div> : (
        <ul className="mt-3 space-y-2">
          {visible.map((alarm) => (
            <li key={alarm.id} className="rounded-lg border border-border-subtle bg-bg-surface p-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-text-primary">{displayText(alarm.title, alarm.id)}</p>
                  <p className="mt-1 text-xs text-text-muted">{displayText(alarm.raisedAt, alarm.assetId)}</p>
                </div>
